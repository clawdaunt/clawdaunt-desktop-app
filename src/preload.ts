...
contextBridge.exposeInMainWorld('api', {
  ...
  waitForRunning: (): Promise<boolean> => {
    return new Promise((resolve) => {
      const handler = (_: unknown, s: string) => {
        if (s === 'running') { ipcRenderer.removeListener('status', handler); resolve(true); }
        if (s === 'error' || s === 'stopped') { ipcRenderer.removeListener('status', handler); resolve(false); }
      };
